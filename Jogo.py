...
def principal(nomecolocado):
    ...
    while jogo_roda:
        ...
        # Cria um carrinho
        carro=[]
        jogador =Jogador(assets['img_jogador'])
        ...
        policial=[]
        policia= Policia(assets['img_policial'])
        ...
        while rodando:
            ...
            if random.randrange(1, 250) == 1:
                estrela = Estrela(assets['img_estrela'])
                all_sprites.add(estrela)
                estrelas.add(estrela)

            # Probabilidade de aparecer "Mais"(+)
            if random.randrange(1,300) == 1:
                mais = Mais(carrega_assets['img_mais'])
                all_sprites.add(mais)
                mais.add(mais)
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if quantidade_tiros > 0:    
                        if event.key == pygame.K_SPACE:
                            ...
                            tiros.add(tiro_)
                            carrega_assets(img, som)['som_tiro'].play()
                            quantidade_tiros -= 1
